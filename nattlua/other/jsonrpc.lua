...
local function check_request(rpc)
    if rpc.jsonrpc ~= VERSION then
        return error_response(nil, JSONRPC_ERRORS.INVALID_REQUEST, "this server only accepts jsonrpc version " .. VERSION)
    end
    ...
    do
        ...
        if t ~= "string" and t ~= "number" and t ~= "nil" then
            return error_response(nil, JSONRPC_ERRORS.INVALID_REQUEST, "id must be a string, number or undefined. got " .. t)
        end
    end
end
