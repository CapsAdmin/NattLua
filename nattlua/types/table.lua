...
function META.IsSubsetOf(A, B)
    ...
    if B.Type == "tuple" then
        if not A:IsNumericallyIndexed() then
            return type_errors.other("cannot compare against tuple when I'm not numerically indexed")
        end
        ...
        if B:GetLength() > 0 then
            for i, a in ipairs(A:GetData()) do
                if a.key.Type == "number" then
                    ...
                    if not b then
                        return type_errors.missing(B, a.key, reason)
                    end
                    ...
                    if not ok then
                        return type_errors.subset(a.val, b, reason)
                    end
                end
            end
        end
    ...
    for _, keyval in ipairs(contract:GetData()) do
        if keyval.key:IsLiteral() then
            ...
            if not ok then
                ...
                if (keyval.val.Type == "symbol" and keyval.val:GetData() == nil) or (keyval.val.Type == "union" and keyval.val:HasNil()) then
                    return true
                end
...
function META:GetKeyVal(key, reverse_subset)
    if not self:GetData()[1] then
        return type_errors.missing(self, key, "table is empty")
    end
...
function META:pairs()
    local i = 1
    return function()
        local keyval = self:GetData() and self:GetData()[i] or self:GetContract() and self:GetContract()[i]
