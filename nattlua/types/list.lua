...
function META:__tostring()
	if self.suppress then return "*self*" end
	self.suppress = true
	local s = {}
	level = level + 1
	...
	self.suppress = nil
	table.sort(s, sort)
	if #self:GetData() == 1 then return "[" .. table.concat(s, ""):gsub("\t", " ") .. " ]" end
	return "[\n" .. table.concat(s, ",\n") .. "\n" .. ("\t"):rep(level) .. "]"
end
		...
		if B:GetLength() > 0 then
			for i, a in ipairs(A:GetData()) do
				if a.key.Type == "number" then
					...
					if not b then return type_errors.missing(B, i, reason) end
					if not a.val:IsSubsetOf(b) then return type_errors.subset(a.val, b) end
				end
			end
		else
			local count = 0
			...
			for i, a in ipairs(A:GetData()) do
				if a.key:GetData() ~= i then return type_errors.other("index " .. tostring(a.key) .. " is not the same as " .. tostring(i)) end
				count = count + 1
			end

			if count ~= B:GetMaxLength() then return type_errors.other(
				" count " .. tostring(count) .. " is not the same as max length " .. tostring(B:GetMaxLength())
			) end
		end
...
function META:ContainsAllKeysIn(contract)
	for _, keyval in ipairs(contract:GetData()) do
		if keyval.key:IsLiteral() then
			...
			if not ok then return type_errors.other(tostring(keyval.key) .. " is missing from " .. tostring(contract)) end
		end
	end
...
function META:Contains(key)
	if self.ElementType then return true end
	return self:GetKeyVal(key, true)
end

function META:GetKeyVal(key, reverse_subset)
	if not self:GetData()[1] then return type_errors.other("list has no definitions") end
	if key.Type ~= "number" then return type_errors.other("cannot index list with " .. tostring(key)) end
	local reasons = {}
...
function META:Get(key)
	...
	if not keyval and self:GetContract() then
		if self:GetContract().ElementType then return self:GetContract().ElementType end
		local keyval, reason = self:GetContract():GetKeyVal(key, true)
		if keyval then return keyval.val end
	end
