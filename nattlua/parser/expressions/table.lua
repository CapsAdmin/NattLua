local function table_spread(parser)
    if not (parser:IsCurrentValue("...") and (parser:IsType("letter", 1) or parser:IsValue("{", 1) or parser:IsValue("(", 1))) then return end
    return parser:Expression("table_spread"):ExpectKeyword("..."):ExpectExpression():End()
end
