...
export const registerSyntax = async (lua: LuaEngine) => {
  ...
  const syntax: languages.IMonarchLanguage = {
    ...
    keywords: mapsToArray([
      syntax_runtime.Keywords,
      syntax_runtime.NonStandardKeywords,
    ]),
    typeKeywords: mapsToArray([
      syntax_typesystem.Keywords,
      syntax_typesystem.NonStandardKeywords,
    ]).concat(["string", "any", "nil", "boolean", "number"]),
    ...
    symwbols: new RegExp(
      "[" +
        escapeRegex(
          arrayUnion(syntax_runtime.Symbols, syntax_typesystem.Symbols).join("")
        ) +
        "]+"
    ),
    symbols: /[=><!~?:&|+\-*\/\^%]+/,
