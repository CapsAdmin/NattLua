...
test("smoke", function()
    local a = run[[local type a = 1337 | 8888]]:GetLocalOrEnvironmentValue(types.Literal("a"), "typesystem")
    equal(2, a:GetLength())
...
test("union operator", function()
    local a = run[[
        ...
        local type c = a | b
    ]]:GetLocalOrEnvironmentValue(types.Literal("c"), "typesystem")
    equal(4, a:GetLength())
end)
...
test("is literal", function()
    ...
    assert(a:GetLocalOrEnvironmentValue(types.Literal("a"), "typesystem"):IsLiteral() == false)
end)
