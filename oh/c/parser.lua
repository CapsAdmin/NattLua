...
do -- statements
    ...
    function META:ReadStatement()
        ...
        if self:IsType("macro") then
            local lex = require("oh.c.lexer")(self:GetToken().value:sub(2))
        ...
        local decl = self:BeginExpression("type_declaration", true)
        decl.qualifiers = {}
        while true do
            if self:IsQualifier() then
                local tk = self:ReadTokenLoose()
                decl.qualifiers[tk.value] = tk
            elseif self:IsType("letter") then
                decl.label = self:ReadTokenLoose()
            else
                break
            end
        end
        local node = self:EndExpression()
