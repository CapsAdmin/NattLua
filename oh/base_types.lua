...
local function get_declarations(k, v)
	...
	for i, args in ipairs(get_overloads(v.args)) do
		local f = k .. " = function" .. args .. ": " .. v.returns
		table.insert(out, f)
	end
	...
	for lib, data in pairs(tbl) do
		...
		elseif data.type == "function" then
			for i,v in ipairs(get_declarations(lib, data)) do
				interfaces[key] = interfaces[key] or {}
				table.insert(interfaces[key], v)
			end
		end
	end
end
...
for lib, functions in pairs(interfaces) do
	local s = "interface " .. lib  .. " {\n\t"
	s = s .. table.concat(functions, "\n\t")
	s = s .. "\n}\n"
