...
local sec = util.MeasureFunction(function()
    local compiler = nl.Code(lua_code, "10mb.lua")

    local tokens = util.Measure("compiler:Lex()", function() return assert(compiler:Lex()).Tokens end) 
    local ast = util.Measure("compiler:Parse()", function() return assert(compiler:Parse()).SyntaxTree end)
