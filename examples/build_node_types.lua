...
local function read_statement(node)
    ...
    if in_node[1] and str:find(in_node[1].local_name .. ".", nil, true) then
        if str:find("tokens%[") then
            ...
            if name then
                in_node[1].tokens[name] = true
            end
        else
            local field = str:match(in_node[1].local_name .. ".([a-z_]+)")
            if field then
                in_node[1].fields[field] = true
            end
        end
    end
...
local function crawl_statement(node)
    ...
    elseif node.kind == "while" or node.kind == "repeat" or node.kind == "generic_for" or node.kind == "numeric_for" or node.kind == "do" then
        for _, node in ipairs(node.statements) do
            crawl_statement(node)
        end
    else
...
local compiler = assert(nl.File("nattlua/parser/parser.lua", {
    inline_require = true,
