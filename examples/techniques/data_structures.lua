...
local function meiyan_hash(key)
    ...
    while count >= 8 do
        h = bit.bxor(h, bit.bxor(bit.bor(bit.lshift(cast32(key), 5), bit.rshift(cast32(key), 27)), cast32(key + 4))) * 0xad3e7;
    ...
    if bit.band(count, 1) ~= 0 then
        h = bit.bxor(h, cast8(key)) * 0xad3e7;
    end
